          ...
          <ScrollReveal key={project.slug} delay={index * 0.15}>
            ...
            >
              <GlassCard hover className="p-6 h-full flex flex-col">
                ...
                {/* Tech stack */}
                <div className="flex flex-wrap gap-2 mb-4" aria-label={`${project.name} tech stack`}>
                  {project.techStack.map((tech) => (
                    <span key={tech} className="tag">{tech}</span>
                  ))}
                </div>

                <span className="inline-flex items-center gap-1 text-accent-blue text-sm mt-auto">
                  Read case study
